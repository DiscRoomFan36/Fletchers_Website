...
func If_Then_Else[T any](condition bool, if_true, if_false T) T {
    if condition { return if_true;
    } else       { return if_false; }
}

const PRETTY_PRINT_JS_VALUES = true;

func js_value_to_string(js_value js.Value) string {
    sb := strings.Builder{};

    // if MAX_DEPTH == -1, go forever,
    const MAX_DEPTH = -1;

    seen_objects := make([]js.Value, 0);

    var recur func(js_value js.Value, depth int);
    recur = func (js_value js.Value, depth int) {

        if (js_value.Type() == js.TypeObject) {
            for _, seen_object := range seen_objects {
                if js_value.Equal(seen_object) {
                    sb.WriteString("(circular reference)");
                    return;
                }
            }

            Append(&seen_objects, js_value);
        }
        ...
        switch js_type {
            ...
            case js.TypeBoolean: {
                as_bool := js_value.Bool();
                sb.WriteString(If_Then_Else(as_bool, "true", "false"));
            }
            case js.TypeNumber: {
                as_float := js_value.Float();
                fmt.Fprintf(&sb, "%f", as_float);
            }
            case js.TypeString: {
                as_string := js_value.String();
                fmt.Fprintf(&sb, "\"%s\"", as_string);
            }

            case js.TypeSymbol: {
                fmt.Fprintf(&sb, "Symbol?");
            }
            case js.TypeObject: {
                ...
                if len(keys) == 0 {
                    ...
                    if PRETTY_PRINT_JS_VALUES {

                        fmt.Fprintf(&sb, "(Object){\n");
                        for i, key := range keys {
                            if i > 0 { sb.WriteString(",\n"); }

                            for range depth+1 { sb.WriteString("    "); }

                            value := js_value.Get(key);
                            fmt.Fprintf(&sb, "%s: ", key);
                            recur(value, depth + 1);
                        }
                        sb.WriteString(",\n");
                        for range depth { sb.WriteString("    "); }
                        sb.WriteString("}");
            ...
            case js.TypeFunction: {
                // doing a thing where I show how many function arguments
                // this function wants.
                //
                // super not perfect.
                //
                // but no way got get the read function argument names,
                // unless on browsers with non strict mode or something.
                // didn't pursue that avenue of thought.
                //
                // what you get is what you get.

                // number of function arguments
                length := js_value.Get("length").Int();

                fmt.Fprintf(&sb, "function(");
                for i := range length {
                    if i > 0 { sb.WriteString(", "); }
                    if i > 26 {
                        // this function has more than 26 arguments, you've got
                        // more problems then not seeing them all here.
                        sb.WriteString("...");
                        break;
                    }
            ...
            default: { panic(fmt.Errorf("unknown js type: %v", js_type)); }
        }
    }
...
func parse_js_value_to_type_helper(js_value js.Value, reflected_type reflect.Value, name string) error {
    ...
    case reflect.Map: {
        ...
        value_type := reflected_type.Type().Elem(); // hope this the value type and not something else...
