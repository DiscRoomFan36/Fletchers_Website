...
type User_Input struct {
    // if would probably be fine to have these as ints, but the
    // setup_input_handling function could return a valid floating
    // point position. so float it is.
    //
    // (its probably fine to convert these to ints in whatever case you have.)
    Mouse_Position            Vec2[float32];
    Mouse_Position_Previously Vec2[float32];
